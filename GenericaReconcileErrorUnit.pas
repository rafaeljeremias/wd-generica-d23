...
unit GenericaReconcileErrorUnit;

interface

uses
  SysUtils, Windows, Variants, Messages, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, Grids, DB, DBClient, Provider, ExtCtrls, WiseUnit;

const
  ActionStr: array[TReconcileAction] of string = ('Pular', 'Abortar', 'Merge',
    'Corrigir', 'Cancelar', 'Refresh');
  UpdateKindStr: array[TUpdateKind] of string = ('Modificado', 'Inserido',
    'Deletado');
  SCaption = 'Erro de Atualização - %s';
  SUnchanged = '<Sem Mudança>';
  SBinary = '(Binary)';
  SAdt = '(ADT)';
  SArray = '(Array)';
...
type
  ...
  private
    ...
    procedure TraduzErro(msg: String);
  public
...
procedure TReconcileErrorForm.TraduzErro(msg : String);
var
  tabela, constraint, campo, aux : String;
  posicao, i         : integer;
begin
  if pos('violation of PRIMARY or UNIQUE KEY', msg) > 0 then
  begin
    posicao := pos('table',msg) + 7;
    tabela := '';
    for i := posicao to length(msg)-1 do
      if msg[i] <> '"' then
        tabela := tabela + msg[i]
      else
        break;
    ErrorMsg.Lines.Add('Ocorreu um erro de duplicidade na tabela: "'+tabela+'"');
    exit;
  end;
  if pos('violation of FOREIGN KEY', msg) > 0 then
  begin
    posicao := pos('constraint',msg) + 12;
    constraint := '';
    for i:= posicao to length(msg)-1 do
      if msg[i] <> '"' then
        constraint := constraint + msg[i]
      else
        break;
    aux := LerINI('FOREIGN KEY',constraint,'',ExtractFilePath(Application.ExeName)+'Erros.msg');
    if aux <> '' then
      ErrorMsg.Lines.Add(aux);
    exit;
  end;
  if pos('validation error for column', msg) > 0 then
  begin
    for i := 29 to length(msg)-1 do
      if msg[i] <> ',' then
        campo := campo + msg[i]
      else
        break;
    ErrorMsg.Lines.Add('Erro de validação no campo: "'+campo+'"');
    exit;
  end;
  if pos('Operation violates CHECK', msg) > 0 then
  begin
    posicao := pos('constraint',msg) + 11;
    constraint := '';
    for i:= posicao to length(msg)-1 do
      if msg[i] <> ' ' then
        constraint := constraint + msg[i]
        ...
        break;
    aux := LerINI('CHECK CONSTRAINT',constraint,'',ExtractFilePath(Application.ExeName)+'Erros.msg');
    if aux <> '' then
      ErrorMsg.Lines.Add(aux);
  end;
end;
...
begin
  ...
  TraduzErro(AdjustLineBreaks(FError.Message,tlbsCRLF));
  if ErrorMsg.Lines.Count > 0 then
    ErrorMsg.Lines.Add('');
  ErrorMsg.Lines.Add(AdjustLineBreaks(FError.Message,tlbsCRLF));
