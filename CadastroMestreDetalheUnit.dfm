object CadastroMestreDetalheForm: TCadastroMestreDetalheForm
  Left = 156
  Top = 153
  Caption = 'CadastroMestreDetalheForm'
  ClientHeight = 377
  ClientWidth = 562
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel: TPanel
    Left = 0
    Top = 0
    Width = 562
    Height = 339
    Align = alClient
    TabOrder = 0
    DesignSize = (
      562
      339)
    object DBGrid1: TDBGrid
      Left = 4
      Top = 144
      Width = 553
      Height = 192
      Hint = 
        '[INS] - Inserir Registro           [Ctrl + Del] - Remover Regist' +
        'ro       [ESC] - Cancelar'
      Anchors = [akLeft, akTop, akRight, akBottom]
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnKeyDown = DBGrid1KeyDown
    end
  end
  object PanelBotao: TPanel
    Left = 0
    Top = 339
    Width = 562
    Height = 38
    Align = alBottom
    TabOrder = 1
    DesignSize = (
      562
      38)
    object GravaButton: TBitBtn
      Left = 404
      Top = 4
      Width = 75
      Height = 29
      Anchors = [akRight, akBottom]
      Caption = '&Grava'
      DoubleBuffered = True
      Glyph.Data = {
        
        6666666666666666666666666666666666666666666666666666}
      ParentDoubleBuffered = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = GravaButtonClick
    end
    object CancelaButton: TBitBtn
      Left = 483
      Top = 4
      Width = 75
      Height = 29
      Anchors = [akRight, akBottom]
      Caption = '&Cancela'
      DoubleBuffered = True
      Glyph.Data = {
        
        0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B}
      ParentDoubleBuffered = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = CancelaButtonClick
    end
    object NovoButton: TBitBtn
      Left = 324
      Top = 4
      Width = 75
      Height = 29
      Anchors = [akRight, akBottom]
      Caption = '&Novo'
      DoubleBuffered = True
      Glyph.Data = {
        
        1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A1A}
      ParentDoubleBuffered = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      Visible = False
      OnClick = GravaButtonClick
    end
  end
  object MestreDataSource: TDataSource
    DataSet = MestreClientDataSet
    Left = 424
    Top = 176
  end
  object MestreClientDataSet: TClientDataSet
    Aggregates = <>
    PacketRecords = 1
    Params = <>
    ProviderName = 'MestreDataSetProvider'
    OnReconcileError = DetalheSimpleDataSetReconcileError
    Left = 320
    Top = 176
  end
  object MestreDataSetProvider: TDataSetProvider
    DataSet = MestreSQLDataSet
    BeforeUpdateRecord = MestreDataSetProviderBeforeUpdateRecord
    Left = 208
    Top = 176
  end
  object MestreSQLDataSet: TSQLDataSet
    GetMetadata = False
    DbxCommandType = 'Dbx.SQL'
    Params = <>
    Left = 52
    Top = 172
  end
  object DetalheSQLDataSet: TSQLDataSet
    GetMetadata = False
    DbxCommandType = 'Dbx.SQL'
    DataSource = DS
    Params = <>
    Left = 52
    Top = 224
  end
  object DS: TDataSource
    DataSet = MestreSQLDataSet
    Left = 124
    Top = 176
  end
  object DetalheClientDataSet: TClientDataSet
    Aggregates = <>
    Params = <>
    OnReconcileError = DetalheSimpleDataSetReconcileError
    Left = 316
    Top = 228
  end
  object DetalheDataSource: TDataSource
    DataSet = DetalheClientDataSet
    Left = 424
    Top = 228
  end
end
